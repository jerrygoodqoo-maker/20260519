...
const LB = { rock: '石頭', paper: '布', scissors: '剪刀', thumbs_up: '繼續', thumbs_down: '結束' };
const BEATS = { rock: 'scissors', scissors: 'paper', paper: 'rock' };
const PAL = ['#00F5FF', '#FF00FF', '#00FF9F', '#00E5FF', '#FF0055', '#7000FF'];
...
function skel() {
    if (!lm) return;
    g.save();
    g.strokeStyle = 'rgba(0, 245, 255, 0.6)'; g.lineWidth = 2;
    SKEL.forEach(([a, b]) => {
    ...
    lm.forEach((p, i) => {
        const [x, y] = lxy(p);
        g.fillStyle = i ? '#00FF9F' : '#FF00FF';
        g.beginPath(); g.arc(x, y, i ? 3 : 5, 0, Math.PI * 2); g.fill();
    });
    g.restore();
}

function boldT(t, x, y, fs, col, stroke, shadow = '#00F5FF') {
    g.save(); g.font = `bold ${fs}px "Courier New", monospace`; g.textAlign = 'center'; g.textBaseline = 'middle';
    g.shadowColor = shadow; g.shadowBlur = 15;
...
function smT(t, x, y, fs, col) {
    g.save(); g.font = `${fs}px "Courier New", monospace`; g.textAlign = 'center'; g.textBaseline = 'middle';
    g.fillStyle = col || 'rgba(255,255,255,.6)'; g.fillText(t, x, y); g.restore();
}

function scoreHUD() {
    g.save();
    const sw = 200, sh = 40, sx = W - sw - 10, sy = 10;
    g.fillStyle = 'rgba(0, 0, 0, 0.7)'; g.strokeStyle = '#00F5FF'; g.lineWidth = 1; rr(sx, sy, sw, sh, 4); g.fill(); g.stroke();
...
function btn(lbl, x, y, w, h, bg) {
    ...
    g.save(); 
    g.fillStyle = hov ? bg : 'rgba(0,0,0,0.6)'; g.strokeStyle = bg; g.lineWidth = 2;
    rr(x, y, w, h, 4); g.fill(); g.stroke();
    g.font = `bold ${Math.floor(h * .38)}px "Courier New"`;
    g.textAlign = 'center'; g.textBaseline = 'middle'; g.shadowBlur = hov ? 15 : 0;
    g.fillStyle = hov ? '#000' : '#FFF'; g.fillText(lbl, x + w / 2, y + h / 2);
    g.restore();
}
...
function drawCyberGrid() {
    ...
    g.save();
    const step = 50, off = (t * 30) % step;
    g.lineWidth = 1;
    g.strokeStyle = 'rgba(0, 255, 255, 0.15)'; // Cyan Grid
    for (let x = off; x < W; x += step) {
        g.beginPath(); g.moveTo(x, 0); g.lineTo(x, H); g.stroke();
    }
    g.strokeStyle = 'rgba(255, 0, 255, 0.15)'; // Magenta Grid
    for (let y = off; y < H; y += step) {
        g.beginPath(); g.moveTo(0, y); g.lineTo(W, y); g.stroke();
    }
    g.fillStyle = 'rgba(0, 0, 0, 0.12)'; // Scanlines effect
...
function dLoading() {
    const t = Date.now() / 1000;
    boldT('LOADING…', W / 2, H / 2 - 24, 26, '#FFF', null, '#4ECDC4');
    ...
    smT('允許攝影機存取,以進行遊戲', W / 2, H / 2 + 94, 14, 'rgba(255,255,255,.35)');
...
function loop() {
    ...
    if (st === 'loading' || st === 'ended') {
        ...
        gr.addColorStop(0, '#1a1040'); gr.addColorStop(1, '#050510');
